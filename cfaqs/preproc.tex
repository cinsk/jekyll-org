...
\A
	불행하게도 그런 방법은 존재하지 않습니다.  (왜냐하면, \TT{typedef}와
        ...
        몇가지 매크로를 (예: \verb+MY_TYPE_DEFINED+)
...
\A
	...
	봤을 때, 주석 자체를 없애기 때문에, 주석 양 옆의 토큰(token)을
        서로 붙이는(pasting) 데 씁니다. 그러나, ANSI 표준에서는
        (\cite{kr1}에서도) 주석(comment)이 공백으로 대체된다고 씌여있기 때문에,
