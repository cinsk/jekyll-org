...
\end{verbatim}
	...
	반드시 캐스팅을 써 주어야 합니다.  (질문 \ql{15.3}을 참고하시기 바랍니다.)
...
\A
	다른 시스템과 똑같습니다.
	NULL은 시스템에 상관없이 항상 0 또는 \TT{((void *)0)}으로
	정의되어 있습니다
...
\A
	...
	포인터가 쓰지지 않는 곳에서 \TT{NULL}을 쓰는 것은, 만약 제대로
	동작한다 할지라도, 쓰면 안됩니다. 
...
\begin{enumerate}
	...
	\item	0 또는 NULL이 함수 인자로 쓰일 경우에는, 그 함수 인자 타입에
	  맞는 포인터로 캐스팅해서 사용합니다.
\end{enumerate}
	이 장의 나머지 질문들은, 대부분 사람들이 널 포인터가 실제로 갖는
...
\Q{5.19} % 5.19 COMPLETE
	시스템 주소 0에 위치해 있느 interrupt vector에 어떻게 접근할 수
...
\A
	...
	(그리고 19장도 참고하기 바랍니다.) 주소 0에 접근하는 것이 당연한
