...
\T
	아래에서, \verb+const_pointer+는 포인터가 상수인 것을 나타내며,
...
% TODO: 다른 질문과 연관성을 꼭 쓰도록. 특히 19장과..
...
\T
	C++ 언어와는 달리, C 언어 표준에서는 \TT{main()}이 재귀적으로 (즉,
        \TT{main}이 다시 \TT{main}을 부르는 경우) 호출되는 것을 막지
...
\end{verbatim}
	...
	설정합니다. (\TT{Xstr()} 매크로가 인자를 확장하고 \TT{Str()}
          매크로가 문자열로 만듭니다.)
...
\A
	...
	소스 리스팅 제어, 구조체 압축(packing), 경고 출력 수준(\TT{lint}의
...
\A
	...
	이 제한은 이미 쓸모없어져 가고 있으므로 (obsolescent), ISO 표준에서는
	없어질 예정입니다.
	...
	\TT{volatile} 타입 qualifier는 주어진 오브젝트가 컴파일러가 의도하지
        ...
        \begin{itemize}
          ...
          \item 둘째, \TT{setjmp}와 \TT{longjmp} 함수를 써서 non-local goto를
            사용하는 코드
          \item 세째, (인터럽크 관련) signal handler에서
            (보통 전역) 변수의 값을 설정할 때.
        \end{itemize}
