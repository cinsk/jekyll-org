...
\A
	...
	들면 \TT{comp.unix.questions}나 \TT{comp.os.msdos.programmer}에
	물어보는 것이 낫습니다.  또는 그러한 그룹의 FAQ 목록을 보는 것도
...
% e.g. #include <bios.h> or #include <dos.h> or #include <conio.h>
	...
	이 FAQ 목록의 확장판(질문 20.40 참고)에서는 여러 시스템에서
...
\A
	...
	\TT{rdchk()}, \TT{open()}이나 \TT{fcntl()}에서 \verb+O_NDELAY+
...
\begin{faq}
\Q{19.3}
	한 자리에 몇 퍼센트가 진행되었는지 계속 표시할 수 있는 방법이나
...
\A
	...
	여러 줄들을 넘겨 버리는 것입니다.
	마지막 수단으로, \TT{system()}을 써서 (질문 19.27 참고)
...
\A
	Unix에서는 popen() (질문 19.30 참고) 함수를 써서 `lp'나 `lpr'
...
\A
	문자를 장치에 보내는 방법을 알고 있다면 (칠질문 19.8) 참고,
...
\A
	...
	한가지 해결책이 될 수 있습니다.  \seealso{12.30, 19.13}
\end{faq}
...
\A
	...
	쓰거나 (질문 19.27 참고), 원본과 사본 파일을 (\TT{fopen()}이나
...
\A
	...
	파일이름과 어떤 와일드카드 패턴을 비교하려면 질문 13.7을
	참고하기 바랍니다.
...
\A
	어렵습니다; 질문 19.31의 내용을 먼저 참고하기 바랍니다.
...
\begin{faq}
\Q{19.33}
	프로세스가 그 프로세스를 호출한 프로세스(parent)의 환경 변수를
	바꿀 수 있나요?
\A
	...
	Unix에서는 프로세스는 자신의 환경만을 고칠 (어떤 시스템들은
	\TT{setenv()}와 \TT{putenv()} 함수를 써서) 수 있고,
...
\A
	...
	V7 Unix와 그 후계들은 \TT{ftime()}이라는 매우 쓸모있는 함수를
	제공하며, 이 함수는 millisecond 단위로 동작합니다.
	또 다른 함수들로는 \TT{clock()}, \TT{delay()}, \TT{gettimeofday()},
	\TT{msleep()}, \TT{nap()}, \TT{napms()}, \TT{nanosleep()},
	\TT{setitimer()}, \TT{sleep()}, \TT{times()}, \TT{usleep()}이
...
\R
	...
	\cite{pcs} \S\ 12 \Page{197--8}, 215--6;
	\cite{posix} \S\ 4.5.2.
\end{faq}
...
\end{verbatim}
	...
	잘못 전달되는 것을 막아 줍니다.  (또 이런 꼴의 코드가 \TT{signal()}을
	부르는 방식에 어긋나지도 않습니다.)

	어떤 시스템에서는 키보드 인터럽트가 터미널 입력 시스템에 따라 영향을
	받기도 합니다; 질문 19.1을 참고하기 바랍니다.
...
\A
	...
	(질문 19.38 참고) SIGFPE를 잡아낼 수도 있습니다.
	\seealso{14.9}
...
\A
	...
	경우입니다.  왜냐하면 ANSI/\cite{iso} C 표준은 --- 
	운영체제가 아닌 언어에
	대한 정의라서 --- 이러한 기능을 전혀 정의하고 있지 않기 때문입니다.
	이러한 분야에 대한 국제 표준은
	POSIX\footnote{IEEE 1003.1, \cite{iso}/IEC 9945-1}에서 다루고 있으며,
