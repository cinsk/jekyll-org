...
이 절의 많은 부분이 \TT{printf} (질문 12.6에서 12.11까지)와 \TT{scanf}
(질문 12.12에서 12.20까지)에 연관되어 있습니다.  질문 12.21에서 12.26은
다른 stdio 함수들을 설명합니다.  특정 파일을 access할 필요가 있다면,
\TT{fopen} (질문 12.27부터 12.32까지)을 써서 직접 열어서(open) 쓰거나(use),
표준 stream을 redirect해서 (질문 12.33에서 12.36까지) 쓸 수 있습니다.
text I/O를 하려는 마음이 없다면, ``binary'' stream을 쓸 수 (질문
12.37에서 12.42까지) 있습니다.   이 모든 것을 자세히 알아보기 전에
...
\R
	...
	\cite{iso} \S\ 6.1.2.5, \S\ 7.9.1, \S\ 7.9.7.5; 
...
\end{verbatim}
	...
	대개 여러분이 \TT{feof()}를 쓸 이유가 없습니다  (종종
...
\R
	...
	\cite{iso} \S\ 7.9.3, \S\ 7.9.7.1, \S\ 7.9.10.2; 
...
\begin{faq}
\q
	제 프로그램의 프롬프트와 중간 단계까지의 출력은 제가 파이프를 써서
	출력을 다른 프로그램에 넘길 때, 때때로 스크린에 출력되지 않습니다.
\A
	출력이 스크린에 반영되기를 원한다면\footnote{다른 방법으로,
	\TT{setbuf()}나 \TT{setvbuf()}를 써서 버퍼링을 쓰지 않은 방법이
	있지만, 버퍼링을 사용하지 않으면 성능이 떨어질 가능성이 높으므로
	그리 좋다고 할 수 없습니다.}
	(특히 텍스트가 \verb+\n+으로
	끝나지 않는 경우), \TT{fflush(stdout)}을 호출해야 합니다.
	대개의 메커니즘이 적절한 시기에 자동으로 \TT{fflush()}를 불러주지만,
	이 메커니즘들은 \TT{stdout}이 터미널이라고 가정한 것이기에
	제대로 동작하지 않을 수 있습니다.  (\seealso{12.24})

% TODO: error() 함수도 참고할 수 있도록 추가할 것.
\T
	질문 \faqref{chap:ext}{faq:error}을 참고하기 바랍니다.
...
\begin{faq}
\q
	\TT{printf}로 `\verb+\%+'를 출력할 수 있을까요? \verb+\%+를 써 보았지만
	출력되지 않더군요.
\A
	\verb+%%+를 쓰면 됩니다.
...
\A
	...
	값) 타입을 할 수 없기 때문입니다.  그러므로 반드시 올바른
...
\A
	...
	\ref{chap:vlal}~절을 참고하기 바랍니다.}를 사용하거나, 
	프로토타입이 없는
...
\A
	\verb+<locale.h>+는 이러한 연산을 수행하기 위한 약간의 기능들을
	제공합니다.  그러나 이런 일을 하기 위한 표준 루틴은 존재하지 않습니다.
	(\TT{printf()}가 할 수 있는 것은 (미리 정의된) 어떤 locale 설정에 따라 
	10진 문자를 출력하는 것만 알 수 있습니다\footnote{
...
\A
	...
	가리키는 위치에 그 값을 저장합니다. (\seealso{20.1})
	따라서 \verb+scanf("%d", &i)+로 써야 합니다.
\end{faq}
...
\A
	...
	\TT{\%hd}를 쓰기 바랍니다. (질문 12.9의 표를 참고하기 바랍니다.)
\end{faq}
...
\begin{faq}
\q % 12.16 COMPLETE
	지정한 형식으로 파일에서 데이터를 읽으려면 어떻게 해야 할까요?
...
\item
	...
	이 장에서 지적한 제한 사항에도 불구하고 (질문 12.20 참고),
...
\begin{verbatim}
  ...
  for (i = 0; i < ac; i++)
    array[i] = atof(av[i]);
\end{verbatim}
	(\TT{makeargv}의 정의는 질문 13.6을 참고하기 바랍니다.)
...
\A
	...
	것이 아니고,
	공백(whitespace) 문자가 나올 때까지의 입력을 읽어서 무시하라는
	뜻입니다.
...
\A
	...
	변환을 중지하고 그 문자 데이터를 \EM{입력 스트림에 그대로 놔 둡니다.}
...
\A
	\TT{scanf()}는 많은 문제를 가지고 있습니다 --- 질문 12.17, 12.18,
	12.19를 참고하기 바랍니다.  또 \TT{gets()}가 가지고 있는 문제가
	\%s 포맷을 사용할 때에도 발생합니다 (질문 12.23 참고) --- 즉
	입력받을 버퍼가 오버플로우가 일어나지 않는다고\footnote{여러분이
	\TT{\%20s}와 같이 폭을 지정한다면 문제가 해결될 수도 있습니다.
	질문 12.15를 참고하기 바랍니다.} 보장할 수 없습니다.
	...
	함수를 써서) 방법을 쓰는 것이 좋습니다; \seealso{12.16, 13.6})
...
\A
	...
	아주 쉽습니다.  질문 7.1을 보면 \TT{gets} 대신 \TT{fgets()}를
	사용한 코드를 볼 수 있습니다.

\T
	머피의 법칙(Murphy's Law)에 관한 것은
	\cite{raymond:jargon}~\를 참고하기 바랍니다.
\R
...
\A
	...
	쓰면 됩니다.  \seealso{12.2, 20.4}

\T
	에러가 발생하지 않았을 때에는
	\TT{errno}가 0이라는 것을 보장할 수 없습니다.
\R
	\cite{iso} \S\ 7.1.4, \S\ 7.9.10.3; 
...
\begin{faq}
\q % 12.25 COMPLETE
	\TT{fgetpos/fsetpos}와 \TT{ftell/fseek}의 차이점은 무엇인가요?
	\TT{fgetpos/fsetpos}는 어디에 쓰는 거죠?
\A
	새로운 \TT{fgetpos/fsetpos} 함수는
	offset을 나타내기 위해 특별한 \TT{typedef}인 \verb+fpos_t+를 
	사용합니다.
...
% 아래 문장은 book version에는 없는 말입니다. (new version)
	...
	다중바이트(multibyte) 스트림에도 쓸 수 있습니다.  \seealso{1.4}
\R
	\cite{kr2} \S\ B1.6 \page{248}; 
	\cite{iso} \S\ 7.9.1, \S\ 7.9.9.1, 7.9.9.3; 
...
\A
	표준 C 언어에서 \TT{fflush()}는 출력 스트림에 대해서만 동작합니다.
	...
	아직 읽지 않은 문자(unread characters)들을
	stdio input stream에서 무시하는(discard)
	표준 방법은
	존재하지 않습니다.
	어떤 회사에서는 \TT{fflush(stdin)}이 읽지 않은 문자들을 취소할 수
	있도록 라이브러리를 만들기도 하지만, 이식성이 뛰어난 프로그램을
	...
	Input을 flush할 방법이 필요하다면 (질문 19.1과 19.2에 나온 것처럼)
...
% 아래 줄은 book version에서는 없는 내용입니다.
	또는 \verb+\n+이 나오기 전까지의 문자를 읽어서 무시하거나 curses
	루틴인 \TT{flushinp()}를 사용하거나, 
	쓰면 됩니다. \seealso{19.1, 19,2}
\R
	\cite{iso} \S\ 7.9.5.2; 
	\cite{hs} \S\ 15.2.
\end{faq}
...
\A
	좋은 해결책은 존재하지 않습니다.  다시 돌아올 필요가 있다면,
...
\end{verbatim}
	...
	바꾸면 됩니다.  (물론 \TT{putchar}나 \TT{puts}를 썼다면
...
\end{verbatim}
	...
	상수(constant)이기 때문에 다른 값으로 설정(assign)할 수
	...
	어떤 시스템에서는 controlling terminal을 (질문 12.36 참고)
	...
	\TT{freopen}은 제대로 동작할 수 없습니다;  질문 19.30을
	대신 참고하기 바랍니다.
\end{faq}
...
\A
	...
	아닌 경우 prompt를 출력하게 않도록 만들고 싶은 경우, 어떤 시스템에서는
...
\A
	...
	MS-DOS에서는 파일 \TT{CON}을 열거나 \TT{getch}와 같은
...
\A
	...
	함수를 만들어 쓰면 됩니다.  질문 15.5를 참고하기 바랍니다.
\end{faq*}
...
\begin{faq}
\q % 12.39 COMPLETE
	Binary file에 쓰기 위한 ``filter''를 만들고 있습니다.
	그런데 \TT{stdin}과 \TT{stdout}은 이미 text stream으로 열려 있는
	상태라서 문제가 됩니다. 어떻게 하면 이 stream을 binary mode로
	바꿀 수 있을까요?
\A
	이런 일을 하기 위한 standard 방법은 없습니다.
	UNIX와 같은 OS는 text/binary를 구별하지 않으므로 mode를 바꿀 필요가
...
\R
	\cite{ansi} \S\ 4.9.2; \cite{iso} \S\ 7.9.2;
...
\end{verbatim}
	...
	(질문 3.14 참고) 보장해주며, \TT{(unsigned)}로 casting한 것은
	부호 확장(sign extension)을 막기 위한 것입니다.
	(일반적으로 이러한 코드를 작성할 때에는 모든 data type을 
	\TT{unsigned}로 하는 것이 좋습니다.  그러나 먼저 질문 3.19를
	참고하기 바랍니다.)
