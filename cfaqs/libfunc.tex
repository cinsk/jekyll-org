...
\A
	...
	(각각 \TT{\%ld}와 \TT{\%f}를 쓰면 됨); 즉, \TT{sprintf}를 
        ...
        꼭 \TT{iota}를 써야겠다면, 다음과 같은 사항들을 고려해야 합니다:
        \begin{itemize}
          \item K\&R은 간단한 버전의 \TT{itoa}를 제공합니다.
          \item 돌려줄 버퍼를 어떻게 할당할 것인지 생각해 봐야 합니다;
            질문 \ql{7.5}를 참고 바랍니다.
          \item 간단히 만들었다면, 보통 가장 적은 음수(\verb+INT_MIN+,
            일반적으로 -32,768 또는 -2,147,483,648)를 올바르게 처리하지 못할
            수도 있습니다.
        \end{itemize}
...
\A
	없습니다. (그 이유 중 하나는, C 언어에는 문자열을 취급하는 전용의
        데이터 타입이 없기 때문입니다. 질문 \ql{7.2}와 Chapter 8을 참고하기
        바랍니다.)
...
\A
	어떤 라이브러리들은 이 역할을 위해, \TT{strupr}와 \TT{strlwr}, 또는
        ...
        아닙니다. \verb+<ctype.h>+에 정의되어 있는 매크로인 \TT{toupper}와
...
\end{verbatim}
	\TT{qsort} 함수에서 쓰이는 비교 함수에 왜 이런 포인터 변환이 필요한지
        ...
        하므로, 정렬 대상을 가리키기 위해, generic 포인터(\TT{void *})를 
...
\A
	이러한 문제를 해결하기 위해서 ANSI/ISO 표준 C에서는
	...
	lgx, \TT{struct tm}의 변수에 기준 날짜 값을 넣고, 
        ...
        (또, 추가적으로 이 값을 \verb+time_t+ 타입으로 변경시켜 줍니다.)
	...
	\TT{int} 값을 넘는 값일 경우 (예를 들어 32,736 이상), 오버플로우가
        발생할 수 있습니다.
	또한 써머 타임(daylight saving time)을 고려하면
...
\end{verbatim}
	\noindent (\verb+tm_isdst+를 -1로 설정하거나 \verb+tm_hour+를
        12로 설정하면 써머 타임(daylight saving time) 문제로부터 보호받는데
        도움을 줍니다.)
...
% TODO: 이 부분 책보고 채울것..
	...
	\noindent 나머지(modulus) 연산이 소수(prime) 연산이기 때문에,
        ...
        여기에 쓰인 곱하기는 Schrage씨에 의해 소개된 기법을 써서, 곱한 결과인
        \TT{aX}가 overflow나는 일은 없습니다. 위 코드는 값의 범위가
...
\A
	...
	상위 (high-order) 비트를 써보기 바랍니다: 질문 \ql{13.16}을 보기 바랍니다.
\R
...
\end{verbatim}
	다른 방법에 대한 것은 이 목록의 화장판(질문 \ql{20.40} 참고)에서
	참고하기 바랍니다.
\R
