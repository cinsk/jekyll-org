...
\T
        ...
        있습니다. 시스템이 지원하는 경우, 임의의 N 비트를 가지는
        부호있는(signed) 정수를 위한 
        \verb+int+\EM{N}\verb+_t+와 부호없는(unsigned) 정수를 위한
        \verb+uint+\EM{N}\verb+_t+가
        제공됩니다. 만약 컴파일러가 지원하는 정수 타입이 각각 8, 16, 32 비트로
        이루어져 있다면, 이 컴파일러는 \verb+int8_t+, \verb+int16_t+, 
        \verb+int32_t+, \verb+uint8_t+, \verb+uint16_t+,
        \verb+uint32_t+를 제공합니다.
