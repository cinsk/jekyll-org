...
\begin{description}
\item[기타 테크닉]     20.1 -- 20.6
\item[비트와 바이트]   20.7 -- 20.12
\item[효율성]          20.12 -- 20.16
\item[switch 문장]     20.16 -- 20.18
\item[기타 언어 기능]  20.19 -- 20.24
\item[다른 언어들]     20.25 -- 20.27
\item[알고리즘]        20.28 -- 20.33
\item[만물상]          20.34 -- 20.40
\end{description}

\section{기타 테크닉}	\label{sec:misctech}
...
\A
	...
	이용하는 방법이 있습니다.  \seealso{2.7, 4.8, 7.5a}
\end{faq}
...
\A
	...
	OSI의 ASN.1 (CCITT X.409와 \cite{iso} 8825 ``Basic Encoding Rules''에서
...
\end{verbatim}
	...
	해당하는 함수 포인터를 호출하면 됩니다.  \seealso{2.15, 18.14, 19.36}
\R
	\cite{pcs} \S\ 11 \page{168}.
\end{faq}

\section{비트와 바이트}	\label{sec:bitbyte}
% from 20.7
...
\A
	...
	바이너리 I/O에 관한 것은 질문 2.11을 참고하기 바랍니다.
	\seealso{8.6, 13.1}

\R
	\cite{iso} \S\ 7.10.1.5, 7.10.1.6.
\end{faq}
...
\A
	...
	써서 직접 문자열을 만들어야 합니다.  \seealso{20.10}
\end{faq}

\section{효율성}	\label{sec:efficiency}
% from 20.12
...
\A
	...
	lookup 테이블을 써서 해결될 수 있습니다 (그러나 질문 20.13을
	꼭 참고하기 바랍니다).
\end{faq}
...
\end{quote}
	...
	Chapter 7, 그리고 Jon Bentley의 \EM{Writing Efficient Programs}를
	참고하기 바랍니다.
\end{faq}
...
\section{\TT{switch} 문장}	\label{sec:switch}
% from 20.16
...
\A
	...
	비교해야 합니다.  \seealso{10.12, 20.18, 20.29}
\R
...
\A
	...
	단일한(single), 상수의(constant), 정수 수식(integral expression)만
...
\section{기타 언어 기능}	\label{sec:misclangfeature}
% from 20.19
...
\A
	\TT{assert()} 함수는 \verb+<assert.h>+에 정의되어 있는 매크로 함수이며
...
\begin{verbatim}
  assert(p != NULL);
\end{verbatim}
	\noindent 만약 `assertion'이 실패하면 프로그램은 강제로 종료됩니다.
...
\section{다른 언어들}	\label{sec:otherlang}
% from 20.25
...
\A
	...
	다 지원합니다.  \seealso{8.9, 20.20}
\R
	\cite{hs} p.\ xviii, \S\ 1.1.5 \page{6}, \S\ 2.8 \Page{36--7}, 
		\S\ 4.9 \Page{104--107}.
\end{faq}

\section{알고리즘}	\label{sec:algorithm}
% from 20.28
...
%	Approximate Pattern-Matching Tool."
	...
	다른 방법으로는 여러 비슷한 단어를 동일한 코드로 매핑시켜주는
...
\A
	\TT{mktime()}과 \TT{localtime()}을 쓰면 됩니다.  (\seealso{13.13, 13.14}.
	그러나 \verb+tm_hour+가 0이 될 때, DST 조정(adjustment)을
...
% TODO: 다음 문장 번역
	...
	\seealso{13.14, 13.14b}
\end{faq}

\section{만물상}	\label{sec:trivia}
% from 20.34
\begin{faq}
\Q{20.34}
...
\A
	이 컨테스트는 항상 진행 중입니다.
	\TT{http://www.ioccc.org/index.html}을 참고하시기 바랍니다.

	수상자는 대개 Usenix 포럼에 공표되며 그 후로 net에도 계시됩니다.
	과거의 (1984년 이후) 수상 작품들은 \TT{ftp.uu.net}의
	\TT{pub/ioccc} 디렉토리에 저장되어 있습니다(질문 18.16 참고);
	\TT{http://www.ioccc.org/index.html}도 참고하기 바랍니다.
\end{faq}
...
\A
	...
	오브젝트로 생각할 수 있습니다.  (그러나 질문 6.7을 참고하기 바랍니다.)
...
\A
	...
	최신 문서는 \TT{comp.lang.c}에 한달 주기로 게시됩니다.
	...
	들면 \TT{rtfm.mit.edu}와 (\TT{pub/usenet/news.answers/C-faq/} 디렉토리)
	\TT{ftp.uu.net}이 (\TT{usenet/news.answers/C-faq} 디렉토리) 있습니다.
